# CODEX IMPLEMENTATION PROMPT — Dashboard, Layout, Navigation
...
Stabilize the application shell, dashboard routing, role dashboard selection, layout rendering, and page-to-JS loading so every user lands on the correct dashboard and navigation does not point to broken pages.
...
MVP workflows:
1. User logs in.
2. User is routed to correct role dashboard.
3. Sidebar renders only permitted items.
4. Clicking sidebar pages loads correct PHP page.
5. Matching JS controller initializes once.
6. Missing pages are either created, redirected, or removed from sidebar.
7. Dashboards show real role-appropriate data or honest empty states.
...
Create/update:
- docs/AI_DOS/DASHBOARD_NAVIGATION_MAP.md
- docs/AI_DOS/DASHBOARD_TESTING_CHECKLIST.md

Implement:
- canonical dashboard registry
- sidebar URL validation cleanup
- dashboard page/controller linking
- role dashboard fallbacks that are safe and explicit, not fake data
