const ClassStreamsController = (() => {
    ...
    async function loadData() {
        try {
            const r = await window.API.apiCall('/academic/class-streams', 'GET');
            allData = r?.data || r || [];
    ...
    function renderStats(data) {
        ...
        document.getElementById('statClasses').textContent = items.length;
        document.getElementById('statStreams').textContent = items.length;
        document.getElementById('statAvg').textContent = items.length;
        document.getElementById('statCapacity').textContent = items.length;
    }
    function renderTable(items) {
        ...
        tbody.innerHTML = items.map((item, i) => '<tr><td>' + (i+1) + '</td>' + Object.keys(item).slice(0, 9-1).map(k => '<td>' + escapeHtml(item[k] || '--') + '</td>').join('') + '</tr>').join('');
    }
    function filterData() {
    ...
    function showAddModal() {
        document.getElementById('formModalTitle').innerHTML = '<i class="fas fa-stream me-2"></i>Add Record';
    ...
    async function deleteRecord(id) {
        ...
        try { await window.API.apiCall('/academic/class-streams/' + id, 'DELETE'); showNotification('Deleted', 'success'); await loadData(); }
        catch (e) { showNotification(e.message || 'Delete failed', 'danger'); }
    }
    function exportCSV() {
        if (!allData.length) return;
        const headers = ['#', 'Class (Grade)', 'Stream Name', 'Class Teacher', 'Students', 'Capacity', 'Utilization %', 'Status', 'Actions'];
        const rows = allData.map(item => Object.values(item).slice(0, headers.length));
        let csv = headers.join(',') + '\n' + rows.map(r => r.map(v => '"' + (v||'') + '"').join(',')).join('\n');
        const a = document.createElement('a'); a.href = URL.createObjectURL(new Blob([csv],{type:'text/csv'})); a.download = 'class_streams.csv'; a.click();
    }
