const CompetitionsController = (() => {
    ...
    async function loadData() {
        try {
            const r = await window.API.apiCall('/activities/competitions', 'GET');
            allData = r?.data || r || [];
    ...
    function renderStats(data) {
        ...
        document.getElementById('statWins').textContent = items.length;
        document.getElementById('statParticipants').textContent = items.length;
    ...
    function showAddModal() {
        document.getElementById('formModalTitle').innerHTML = '<i class="fas fa-trophy me-2"></i>Add Record';
    ...
    function exportCSV() {
        ...
        const headers = ['#', 'Competition', 'Category', 'Date', 'Venue', 'Participants', 'Result', 'Award', 'Actions'];
        const rows = allData.map(item => Object.values(item).slice(0, headers.length));
        let csv = headers.join(',') + '\n' + rows.map(r => r.map(v => '"' + (v||'') + '"').join(',')).join('\n');
        const a = document.createElement('a'); a.href = URL.createObjectURL(new Blob([csv],{type:'text/csv'})); a.download = 'competitions.csv'; a.click();
    }
