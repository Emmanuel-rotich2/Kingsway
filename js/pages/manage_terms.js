const ManageTermsController = (() => {
    ...
    async function loadData() {
        try {
            const r = await window.API.apiCall('/academic/terms', 'GET');
            allData = r?.data || r || [];
    ...
    function renderStats(data) {
        ...
        document.getElementById('statCompleted').textContent = items.length;
    ...
    function showAddModal() {
        document.getElementById('formModalTitle').innerHTML = '<i class="fas fa-list-ol me-2"></i>Add Record';
    ...
    async function deleteRecord(id) {
        ...
        try { await window.API.apiCall('/academic/terms/' + id, 'DELETE'); showNotification('Deleted', 'success'); await loadData(); }
        catch (e) { showNotification(e.message || 'Delete failed', 'danger'); }
    }
    function exportCSV() {
        if (!allData.length) return;
        const headers = ['#', 'Term Name', 'Academic Year', 'Start Date', 'End Date', 'Weeks', 'Status', 'Actions'];
        const rows = allData.map(item => Object.values(item).slice(0, headers.length));
        let csv = headers.join(',') + '\n' + rows.map(r => r.map(v => '"' + (v||'') + '"').join(',')).join('\n');
        const a = document.createElement('a'); a.href = URL.createObjectURL(new Blob([csv],{type:'text/csv'})); a.download = 'manage_terms.csv'; a.click();
    }
