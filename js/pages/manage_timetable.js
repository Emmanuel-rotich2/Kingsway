...
const timetableController = (() => {
  ...
  async function loadTeacherCandidates(subjectSelect) {
    ...
    try {
      ...
      teacherSelect.innerHTML = '<option value="">Teacher</option>' + rows.map((candidate) => `<option value="${htmlEscape(candidate.teacher_id)}">${htmlEscape(candidate.teacher_name)} · ${htmlEscape(candidate.eligibility_reason || 'Eligible')}</option>`).join('');
      if (currentTeacher && !rows.some((candidate) => String(candidate.teacher_id) === currentTeacher)) {
        teacherSelect.insertAdjacentHTML('beforeend', `<option value="${htmlEscape(currentTeacher)}" selected>Existing allocation — requires eligibility review</option>`);
      } else if (currentTeacher) teacherSelect.value = currentTeacher;
      const guidance = teacherSelect.closest('td')?.querySelector('.matrix-teacher-guidance');
      if (guidance) guidance.textContent = rows.length ? `${rows.length} approved candidate${rows.length === 1 ? '' : 's'} · suggestions are server-authorized` : 'No approved eligible teacher; qualification review is required.';
    } catch (error) {
      teacherSelect.innerHTML = '<option value="">No eligible teachers loaded</option>';
      if (currentTeacher) teacherSelect.insertAdjacentHTML('beforeend', `<option value="${htmlEscape(currentTeacher)}" selected>Existing allocation — validate before save</option>`);
      const guidance = teacherSelect.closest('td')?.querySelector('.matrix-teacher-guidance');
      if (guidance) guidance.textContent = 'Unable to load eligibility suggestions; server validation remains authoritative.';
    }
  }
  ...
  function renderAiTimetableSession(session) { const host=document.getElementById('matrixAiResponse'); if(!host)return; const messages=Array.isArray(session?.messages)?session.messages:[]; const assistant=messages.filter(m=>m.actor_type==='assistant'&&m.ai_draft).at(-1); if(!assistant){host.classList.add('d-none');return false;} const ai=assistant.ai_draft||{}; const draft=ai.draft||{}; const questions=Array.isArray(draft.questions)?`<div><strong>Questions</strong><ul class="small mb-2">${draft.questions.map(q=>`<li>${htmlEscape(q)}</li>`).join('')}</ul></div>`:''; const suggestions=Array.isArray(draft.suggestions)?`<div><strong>Suggestions</strong><ul class="small mb-2">${draft.suggestions.map(q=>`<li>${htmlEscape(q)}</li>`).join('')}</ul></div>`:''; const assignments=Array.isArray(draft.assignments)?draft.assignments:[]; const apply=assignments.length?`<button type="button" class="btn btn-sm btn-outline-success" id="applyAiTimetableAssignments">Apply ${assignments.length} proposed assignments to editable matrix</button>`:''; host.innerHTML=`<div class="d-flex justify-content-between gap-2"><strong><i class="bi bi-stars text-success me-1"></i>${htmlEscape(draft.title||'AI timetable planning guidance')}</strong><span class="badge text-bg-secondary">${htmlEscape(ai.status||'pending_approval')}</span></div><p class="small mt-2 mb-2">${htmlEscape(draft.body||'Review the assistant guidance, then edit the timetable matrix below.')}</p>${questions}${suggestions}${apply}<div class="small text-muted mt-2">AI suggestions are advisory. Applying them only fills the editable matrix; server eligibility, conflict checks, human approval, and publication remain mandatory.</div>`; host.classList.remove('d-none'); host.querySelector('#applyAiTimetableAssignments')?.addEventListener('click',()=>{ renderMatrix(document.getElementById('matrixScope')?.value || (lowerPrimaryClassTeacher ? 'lower_primary' : 'upper_primary'), assignments); showNotification('AI assignments copied into the editable matrix. Review every cell before saving.','info'); }); return true; }
  async function pollAiTimetableSession(sessionId) { for(let attempt=0;attempt<8;attempt+=1){ try { const response=await API.schedules.getAiTimetablePlanningSession(sessionId); const session=response?.data||response||{}; if(renderAiTimetableSession(session))return; } catch(_){} await new Promise(resolve=>setTimeout(resolve,1000)); } }
  async function askAiTimetableQuestions() { const scope=document.getElementById('matrixScope')?.value || (lowerPrimaryClassTeacher ? 'lower_primary' : 'upper_primary'); const constraints=document.getElementById('matrixAiConstraints')?.value?.trim() || ''; const areas=[...new Set(draftStreams.flatMap((s)=>(s.learning_areas||[]).map((a)=>a.name)).filter(Boolean))]; const activeSlots=timeSlots.filter((s)=>!s.isBreak).slice(0,30); const candidates=[]; outer: for(const stream of draftStreams){ for(const area of (stream.learning_areas||[])){ for(const day of [1,2,3,4,5]){ for(const slot of activeSlots){ const teacher=teachers.find((t)=>Number(t.id||t.staff_id)>0); if(!teacher)continue; candidates.push({academic_year_class_stream_id:Number(stream.academic_year_class_stream_id),learning_area_id:Number(area.id),teacher_id:Number(teacher.id||teacher.staff_id),day_of_week:day,time_slot_id:Number(slot.id||slot.period)}); if(candidates.length>=60)break outer; } } } } const grid=document.getElementById('matrixGrid'); try { const result=await API.schedules.queueAiTimetablePlanning({session_id:Number(grid?.dataset.aiSessionId||0)||undefined,message:constraints||'Prepare a bounded editable timetable assignment proposal and ask for any missing constraints.',class_band:scope,class_count:draftStreams.length,learning_areas:areas,period_count:activeSlots.length,assignment_candidates:candidates,user_constraints:constraints?[constraints]:[]}); const payload=result?.data || result || {}; const sessionId=payload.session_id || payload.data?.session_id || ''; if (sessionId && grid) grid.dataset.aiSessionId=String(sessionId); const jobId=payload.job_id || payload.data?.job_id || ''; document.getElementById('matrixDraftStatus').textContent=sessionId?`AI planning session #${sessionId} queued${jobId?` (job #${jobId})`:''}`:'AI planning request queued'; showNotification('AI planning is now a continuing conversation. Review its proposal before saving the editable matrix.','success'); if(sessionId) await pollAiTimetableSession(sessionId); } catch(e) { showNotification(e.message || 'Unable to queue AI timetable planning questions','danger'); } }
