const ManageCalendarEventsController = (() => {
    ...
    async function loadData() {
        try {
            const r = await window.API.apiCall('/academic/calendar-events', 'GET');
            allData = r?.data || r || [];
    ...
    function renderStats(data) {
        ...
        document.getElementById('statMonth').textContent = items.length;
        document.getElementById('statPast').textContent = items.length;
    }
    function renderTable(items) {
        const tbody = document.querySelector('#dataTable tbody');
        if (!items.length) { tbody.innerHTML = '<tr><td colspan="8" class="text-center text-muted py-4">No records found</td></tr>'; return; }
        tbody.innerHTML = items.map((item, i) => '<tr><td>' + (i+1) + '</td>' + Object.keys(item).slice(0, 8-1).map(k => '<td>' + escapeHtml(item[k] || '--') + '</td>').join('') + '</tr>').join('');
    }
    function filterData() {
    ...
    function showAddModal() {
        document.getElementById('formModalTitle').innerHTML = '<i class="fas fa-calendar-plus me-2"></i>Add Record';
    ...
    async function deleteRecord(id) {
        ...
        try { await window.API.apiCall('/academic/calendar-events/' + id, 'DELETE'); showNotification('Deleted', 'success'); await loadData(); }
        catch (e) { showNotification(e.message || 'Delete failed', 'danger'); }
    }
    function exportCSV() {
        if (!allData.length) return;
        const headers = ['#', 'Event Name', 'Date', 'Time', 'Category', 'Location', 'Description', 'Actions'];
        const rows = allData.map(item => Object.values(item).slice(0, headers.length));
        let csv = headers.join(',') + '\n' + rows.map(r => r.map(v => '"' + (v||'') + '"').join(',')).join('\n');
        const a = document.createElement('a'); a.href = URL.createObjectURL(new Blob([csv],{type:'text/csv'})); a.download = 'manage_calendar_events.csv'; a.click();
    }
