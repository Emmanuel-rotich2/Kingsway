const ParentFeedbackController = (() => {
    ...
    async function loadData() {
        try {
            const r = await window.API.apiCall('/communications/parent-feedback', 'GET');
            allData = r?.data || r || [];
    ...
    function renderStats(data) {
        ...
        document.getElementById('statPositive').textContent = items.length;
        document.getElementById('statNegative').textContent = items.length;
    }
    ...
    function exportCSV() {
        ...
        const headers = ['#', 'Parent', 'Student', 'Category', 'Rating', 'Message', 'Date', 'Status', 'Actions'];
        const rows = allData.map(item => Object.values(item).slice(0, headers.length));
        let csv = headers.join(',') + '\n' + rows.map(r => r.map(v => '"' + (v||'') + '"').join(',')).join('\n');
        const a = document.createElement('a'); a.href = URL.createObjectURL(new Blob([csv],{type:'text/csv'})); a.download = 'parent_feedback.csv'; a.click();
    }
