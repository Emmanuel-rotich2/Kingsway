const CurrentAcademicYearController = (() => {
    ...
    async function loadData() {
        try {
            const r = await window.API.apiCall('/academic/current-year', 'GET');
            allData = r?.data || r || [];
    ...
    function renderStats(data) {
        ...
        document.getElementById('statYear').textContent = items.length;
        document.getElementById('statTerm').textContent = items.length;
        document.getElementById('statWeeks').textContent = items.length;
        document.getElementById('statDays').textContent = items.length;
    }
    function renderTable(items) {
        ...
        tbody.innerHTML = items.map((item, i) => '<tr><td>' + (i+1) + '</td>' + Object.keys(item).slice(0, 6-1).map(k => '<td>' + escapeHtml(item[k] || '--') + '</td>').join('') + '</tr>').join('');
    }
    function filterData() {
