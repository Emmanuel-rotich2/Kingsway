const ComparativeReportsController = (() => {
    ...
    async function loadData() {
        try {
            const r = await window.API.apiCall('/academic/comparative-reports', 'GET');
            allData = r?.data || r || [];
    ...
    function renderStats(data) {
        ...
        document.getElementById('statBestClass').textContent = items.length;
        document.getElementById('statImproved').textContent = items.length;
        document.getElementById('statLeaders').textContent = items.length;
    ...
    function exportCSV() {
        ...
        const a = document.createElement('a'); a.href = URL.createObjectURL(new Blob([csv],{type:'text/csv'})); a.download = 'comparative_reports.csv'; a.click();
    }
