const PerformanceAnalysisController = (() => {
    ...
    async function loadData() {
        try {
            const r = await window.API.apiCall('/academic/performance-analysis', 'GET');
            allData = r?.data || r || [];
    ...
    function renderStats(data) {
        ...
        document.getElementById('statMean').textContent = items.length;
        document.getElementById('statBestSubject').textContent = items.length;
        document.getElementById('statAboveAvg').textContent = items.length;
        document.getElementById('statBelowAvg').textContent = items.length;
    }
    ...
    function exportCSV() {
        ...
        const a = document.createElement('a'); a.href = URL.createObjectURL(new Blob([csv],{type:'text/csv'})); a.download = 'performance_analysis.csv'; a.click();
    }
