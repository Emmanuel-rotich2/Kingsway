const ClassCapacityController = (() => {
    ...
    async function loadData() {
        try {
            const r = await window.API.apiCall('/academic/class-capacity', 'GET');
            allData = r?.data || r || [];
    ...
    function renderStats(data) {
        ...
        document.getElementById('statEnrolled').textContent = items.length;
        document.getElementById('statAvailable').textContent = items.length;
        document.getElementById('statUtil').textContent = items.length;
    }
    ...
    function exportCSV() {
        ...
        const a = document.createElement('a'); a.href = URL.createObjectURL(new Blob([csv],{type:'text/csv'})); a.download = 'class_capacity.csv'; a.click();
    }
