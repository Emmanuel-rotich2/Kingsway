const AssembliesController = (() => {
    ...
    async function loadData() {
        try {
            const r = await window.API.apiCall('/activities/assemblies', 'GET');
            allData = r?.data || r || [];
    ...
    function renderStats(data) {
        ...
        document.getElementById('statTerm').textContent = items.length;
        document.getElementById('statUpcoming').textContent = items.length;
        document.getElementById('statSpeakers').textContent = items.length;
    }
    ...
    function showAddModal() {
        document.getElementById('formModalTitle').innerHTML = '<i class="fas fa-bullhorn me-2"></i>Add Record';
    ...
    async function deleteRecord(id) {
        ...
        try { await window.API.apiCall('/activities/assemblies/' + id, 'DELETE'); showNotification('Deleted', 'success'); await loadData(); }
        catch (e) { showNotification(e.message || 'Delete failed', 'danger'); }
    }
    function exportCSV() {
        if (!allData.length) return;
        const headers = ['#', 'Date', 'Day', 'Time', 'Theme/Topic', 'Speaker', 'Class Responsible', 'Type', 'Actions'];
        const rows = allData.map(item => Object.values(item).slice(0, headers.length));
        let csv = headers.join(',') + '\n' + rows.map(r => r.map(v => '"' + (v||'') + '"').join(',')).join('\n');
        const a = document.createElement('a'); a.href = URL.createObjectURL(new Blob([csv],{type:'text/csv'})); a.download = 'assemblies.csv'; a.click();
    }
