const AdmissionStatusController = (() => {
    ...
    async function loadData() {
        try {
            const r = await window.API.apiCall('/students/admission-status', 'GET');
            allData = r?.data || r || [];
    ...
    function renderStats(data) {
        ...
        document.getElementById('statApproved').textContent = items.length;
        document.getElementById('statRejected').textContent = items.length;
    }
    ...
    function exportCSV() {
        ...
        const headers = ['#', 'Application #', 'Student Name', 'Grade', 'Date Applied', 'Previous School', 'Status', 'Actions'];
        const rows = allData.map(item => Object.values(item).slice(0, headers.length));
        let csv = headers.join(',') + '\n' + rows.map(r => r.map(v => '"' + (v||'') + '"').join(',')).join('\n');
        const a = document.createElement('a'); a.href = URL.createObjectURL(new Blob([csv],{type:'text/csv'})); a.download = 'admission_status.csv'; a.click();
    }
