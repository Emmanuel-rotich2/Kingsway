...
const DashboardRouter = {
    ...
    ROLE_DASHBOARD_MAP: {
        ...
        8: {
            ...
            controller: 'subjectTeacherDashboardController',
            file: 'subject_teacher_dashboard.js',
            scope: 'teaching',
            description: 'My Subject, Exams, Assessments, Grades'
        },
        9: {
            name: 'Intern/Student Teacher',
            controller: 'internTeacherDashboardController',
            file: 'intern_teacher_dashboard.js',
            scope: 'teaching',
            description: 'Limited Teaching, Lesson Observation'
        },
        10: {
            name: 'Accountant',
            controller: 'accountantDashboardController',
            file: 'accountant_dashboard.js',
            scope: 'finance',
        ...
        try {
            ...
            if (typeof AuthContext !== 'undefined' && AuthContext.isAuthenticated()) {
                ...
                if (user) {
                    ...
                    if (user.role_ids) {
                        const roleIds = Array.isArray(user.role_ids) ? user.role_ids : [user.role_ids];
            ...
            if (userJson) {
                ...
                if (user.roles && Array.isArray(user.roles)) {
                    const roleIds = user.roles
                        ...
                        .filter(id => id !== null && id !== undefined);
                    if (roleIds.length > 0) {
                        console.log('✓ Extracted role_ids from sessionStorage roles:', roleIds);
                        return roleIds;
                    }
                }
    ...
    getPrimaryRole: function(roleIds) {
        ...
        const hierarchy = [2, 3, 4, 5, 6, 63, 21, 18, 16, 14, 10, 24, 23, 8, 7, 9, 32, 33, 34];
    ...
    isControllerLoaded: function(controllerName) {
        return typeof window[controllerName] !== 'undefined' && 
    ...
    routeToDashboard: async function() {
        ...
        try {
            ...
            if (!dashboardConfig) {
                console.error(`❌ No dashboard configured for role ID ${primaryRoleId}`);
                this.showErrorPage(`No dashboard available for role ID ${primaryRoleId}`);
                return;
            }
            ...
            try {
                ...
                console.warn(`⚠️ Could not load ${dashboardConfig.file}: ${error.message}`);
            ...
            if (!this.isControllerLoaded(dashboardConfig.controller)) {
                ...
                this.showErrorPage(`Dashboard controller not loaded: ${dashboardConfig.controller}`);
                return;
            }
                ...
                <div class="btn-group ms-auto" role="group">
                    ...
                    <ul class="dropdown-menu dropdown-menu-end" id="roleSwitcherMenu">
                        ${roleIds.map(roleId => {
                            ...
                            const isActive = roleId === currentRoleId ? 'active' : '';
                            return `
                                <li>
                                    <a class="dropdown-item ${isActive}" href="#" data-role-id="${roleId}">
                                        <i class="bi bi-check-circle${isActive ? '-fill' : ''}"></i>
                                        ${config.name}
                                    </a>
                                </li>
                            `;
    ...
    getDashboardInfo: function() {
        return {
            ...
            isMultiRole: window.CURRENT_DASHBOARD_ROLES && window.CURRENT_DASHBOARD_ROLES.length > 1
        };
    }
};
