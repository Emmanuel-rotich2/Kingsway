...
const ENDPOINT_PERMISSIONS = {
    ...
    '/students/student': { 'GET': 'students_view', 'POST': 'students_create', 'PUT': 'students_update', 'DELETE': 'students_delete' },

    // Academic
    '/academic/index': 'academic_view',
    '/academic/curriculum': { 'GET': 'academic_view', 'POST': 'academic_create', 'PUT': 'academic_update' },

    // Attendance
    '/attendance/index': 'attendance_view',
    '/attendance/student': { 'GET': 'attendance_view', 'POST': 'attendance_create', 'PUT': 'attendance_update' },

    // Finance
    '/finance/index': 'finance_view',
    '/finance/payroll': { 'GET': 'finance_view', 'POST': 'finance_create', 'PUT': 'finance_update' },

    // Staff
    '/staff/index': 'staff_view',
    '/staff/staff': { 'GET': 'staff_view', 'POST': 'staff_create', 'PUT': 'staff_update', 'DELETE': 'staff_delete' },

    // Activities
    '/activities/index': 'activities_view',
    '/activities/activity': { 'GET': 'activities_view', 'POST': 'activities_create', 'PUT': 'activities_update' },

    // Inventory
    '/inventory/index': 'inventory_view',
    '/inventory/item': { 'GET': 'inventory_view', 'POST': 'inventory_create', 'PUT': 'inventory_update', 'DELETE': 'inventory_delete' },

    // Admission
    '/admission/index': 'admission_view',
    '/admission/application': { 'GET': 'admission_view', 'POST': 'admission_create', 'PUT': 'admission_update' },

    // Communications
    '/communications/index': 'communications_view',
    '/communications/sms': { 'GET': 'communications_view', 'POST': 'communications_create' },

    // Transport
    '/transport/index': 'transport_view',
    '/transport/route': { 'GET': 'transport_view', 'POST': 'transport_create', 'PUT': 'transport_update' },

    // Schedules
    '/schedules/index': 'schedules_view',
    '/schedules/timetable': { 'GET': 'schedules_view', 'POST': 'schedules_create', 'PUT': 'schedules_update' },
    ...
    '/schoolconfig/config': { 'GET': 'schoolconfig_view', 'PUT': 'schoolconfig_update' },
};
...
window.API = {
    ...
    auth: {
        ...
        login: async (username, password) => {
            ...
            if (response && response.token) {
                ...
                console.log(
                    `User logged in: ${userData.username} with ${AuthContext.getPermissionCount()} permissions`,
    ...
    users: {
        index: async () =>
            apiCall('/users/index', 'GET'),
        get: async (id = null) =>
            id ? apiCall(`/users/user/${id}`, 'GET') : apiCall('/users/user', 'GET'),
        create: async (data) =>
            apiCall('/users/user', 'POST', data),
        update: async (id, data) =>
            apiCall(`/users/user/${id}`, 'PUT', data),
        delete: async (id) =>
            apiCall(`/users/user/${id}`, 'DELETE'),
        bulkCreate: async (users) =>
            apiCall('/users/bulk-create', 'POST', { users }),
        
        // Profile
        getProfile: async (id = null) =>
            id ? apiCall(`/users/profile-get/${id}`, 'GET') : apiCall('/users/profile-get', 'GET'),
        ...
        getRoles: async (id = null) =>
            id ? apiCall(`/users/roles-get/${id}`, 'GET') : apiCall('/users/roles-get', 'GET'),
        ...
        assignRoleToUser: async (userId, roleId) =>
            apiCall('/users/role-assign-to-user', 'POST', { user_id: userId, role_id: roleId }),
        revokeRoleFromUser: async (userId, roleId) =>
            apiCall('/users/role-revoke-from-user', 'DELETE', { user_id: userId, role_id: roleId }),
        
        // Permissions
        getPermissions: async (id = null) =>
            id ? apiCall(`/users/permissions-get/${id}`, 'GET') : apiCall('/users/permissions-get', 'GET'),
        updatePermissions: async (userId, permissions) =>
            apiCall('/users/permissions-update', 'PUT', { user_id: userId, permissions }),
        assignPermission: async (userId, permissionData) =>
            apiCall('/users/permission-assign', 'POST', { user_id: userId, ...permissionData }),
        assignPermissionToUserDirect: async (userId, permissionId) =>
            apiCall('/users/permission-assign-to-user-direct', 'POST', { user_id: userId, permission_id: permissionId }),
        revokePermissionFromUserDirect: async (userId, permissionId) =>
            apiCall('/users/permission-revoke-from-user-direct', 'DELETE', { user_id: userId, permission_id: permissionId }),
        assignPermissionToRole: async (roleId, permissionId) =>
            apiCall('/users/permission-assign-to-role', 'POST', { role_id: roleId, permission_id: permissionId }),
        revokePermissionFromRole: async (roleId, permissionId) =>
            apiCall('/users/permission-revoke-from-role', 'DELETE', { role_id: roleId, permission_id: permissionId }),
        ...
        bulkAssignRolesToUser: async (userId, roleIds) =>
            apiCall('/users/roles-bulk-assign-to-user', 'POST', { user_id: userId, role_ids: roleIds }),
        bulkRevokeRolesFromUser: async (userId, roleIds) =>
            apiCall('/users/roles-bulk-revoke-from-user', 'DELETE', { user_id: userId, role_ids: roleIds }),
        bulkAssignUsersToRole: async (roleId, userIds) =>
            apiCall('/users/users-bulk-assign-to-role', 'POST', { role_id: roleId, user_ids: userIds }),
        bulkRevokeUsersFromRole: async (roleId, userIds) =>
            apiCall('/users/users-bulk-revoke-from-role', 'DELETE', { role_id: roleId, user_ids: userIds }),
        
        // Bulk operations - Permissions
        bulkAssignPermissionsToRole: async (roleId, permissionIds) =>
            apiCall('/users/permissions-bulk-assign-to-role', 'POST', { role_id: roleId, permission_ids: permissionIds }),
        bulkRevokePermissionsFromRole: async (roleId, permissionIds) =>
            apiCall('/users/permissions-bulk-revoke-from-role', 'DELETE', { role_id: roleId, permission_ids: permissionIds }),
        bulkAssignPermissionsToUser: async (userId, permissionIds) =>
            apiCall('/users/permissions-bulk-assign-to-user', 'POST', { user_id: userId, permission_ids: permissionIds }),
        bulkRevokePermissionsFromUser: async (userId, permissionIds) =>
            apiCall('/users/permissions-bulk-revoke-from-user', 'DELETE', { user_id: userId, permission_ids: permissionIds }),
        bulkAssignPermissionsToUserDirect: async (userId, permissionIds) =>
            apiCall('/users/permissions-bulk-assign-to-user-direct', 'POST', { user_id: userId, permission_ids: permissionIds }),
        bulkRevokePermissionsFromUserDirect: async (userId, permissionIds) =>
            apiCall('/users/permissions-bulk-revoke-from-user-direct', 'DELETE', { user_id: userId, permission_ids: permissionIds }),
        bulkAssignUsersToPermission: async (permissionId, userIds) =>
            apiCall('/users/users-bulk-assign-to-permission', 'POST', { permission_id: permissionId, user_ids: userIds }),
        bulkRevokeUsersFromPermission: async (permissionId, userIds) =>
            apiCall('/users/users-bulk-revoke-from-permission', 'DELETE', { permission_id: permissionId, user_ids: userIds }),
    ...
    students: {
        index: async () =>
            apiCall('/students/index', 'GET'),
        
        // CRUD
        get: async (id = null) =>
            id ? apiCall(`/students/student/${id}`, 'GET') : apiCall('/students/student', 'GET'),
        create: async (data) =>
            apiCall('/students/student', 'POST', data),
        update: async (id, data) =>
            apiCall(`/students/student/${id}`, 'PUT', data),
        delete: async (id) =>
            apiCall(`/students/student/${id}`, 'DELETE'),
        
        // Media
        uploadMedia: async (formData) =>
            apiCall('/students/media-upload', 'POST', formData, {}, { isFile: true }),
        getMedia: async (id) =>
            apiCall(`/students/media?student_id=${id}`, 'GET'),
        deleteMedia: async (mediaId) =>
            apiCall('/students/media-delete', 'POST', { media_id: mediaId }),
        
        // Profile & Info
        getProfile: async (id = null) =>
            id ? apiCall(`/students/profile-get/${id}`, 'GET') : apiCall('/students/profile-get', 'GET'),
        getAttendance: async (id = null) =>
            id ? apiCall(`/students/attendance-get/${id}`, 'GET') : apiCall('/students/attendance-get', 'GET'),
        getPerformance: async (id = null) =>
            id ? apiCall(`/students/performance-get/${id}`, 'GET') : apiCall('/students/performance-get', 'GET'),
        getFees: async (id = null) =>
            id ? apiCall(`/students/fees-get/${id}`, 'GET') : apiCall('/students/fees-get', 'GET'),
        getQrInfo: async (id = null) =>
            id ? apiCall(`/students/qr-info-get/${id}`, 'GET') : apiCall('/students/qr-info-get', 'GET'),
        getStatistics: async (id = null) =>
            id ? apiCall(`/students/statistics-get/${id}`, 'GET') : apiCall('/students/statistics-get', 'GET'),
        ...
        generateEnhancedQrCode: async (studentId) =>
            apiCall('/students/qr-code-generate-enhanced', 'POST', { student_id: studentId }),
        ...
        generateClassIdCards: async (classId) =>
            apiCall('/students/id-card-generate-class', 'POST', { class_id: classId }),
        ...
        verifyDocuments: async (studentId, data) =>
            apiCall('/students/admission-verify-documents', 'POST', { student_id: studentId, ...data }),
        conductInterview: async (studentId, data) =>
            apiCall('/students/admission-conduct-interview', 'POST', { student_id: studentId, ...data }),
        approveAdmission: async (studentId, data) =>
            apiCall('/students/admission-approve', 'POST', { student_id: studentId, ...data }),
        completeRegistration: async (studentId, data) =>
            apiCall('/students/admission-complete-registration', 'POST', { student_id: studentId, ...data }),
        getAdmissionWorkflowStatus: async (studentId) =>
            apiCall(`/students/admission-workflow-status?student_id=${studentId}`, 'GET'),
        ...
        verifyTransferEligibility: async (transferId, data) =>
            apiCall('/students/transfer-verify-eligibility', 'POST', { transfer_id: transferId, ...data }),
        approveTransfer: async (transferId, data) =>
            apiCall('/students/transfer-approve', 'POST', { transfer_id: transferId, ...data }),
        executeTransfer: async (transferId, data) =>
            apiCall('/students/transfer-execute', 'POST', { transfer_id: transferId, ...data }),
        getTransferWorkflowStatus: async (transferId) =>
            apiCall(`/students/transfer-workflow-status?transfer_id=${transferId}`, 'GET'),
        ...
        getParents: async (id = null) =>
            id ? apiCall(`/students/parents-get/${id}`, 'GET') : apiCall('/students/parents-get', 'GET'),
        addParent: async (studentId, data) =>
            apiCall('/students/parents-add', 'POST', { student_id: studentId, ...data }),
        updateParent: async (parentId, data) =>
            apiCall(`/students/parents-update/${parentId}`, 'PUT', data),
        removeParent: async (studentId, parentId) =>
            apiCall('/students/parents-remove', 'POST', { student_id: studentId, parent_id: parentId }),
        
        // Medical
        getMedical: async (id = null) =>
            id ? apiCall(`/students/medical-get/${id}`, 'GET') : apiCall('/students/medical-get', 'GET'),
        addMedical: async (studentId, data) =>
            apiCall('/students/medical-add', 'POST', { student_id: studentId, ...data }),
        updateMedical: async (medicalId, data) =>
            apiCall(`/students/medical-update/${medicalId}`, 'PUT', data),
        
        // Discipline
        getDiscipline: async (id = null) =>
            id ? apiCall(`/students/discipline-get/${id}`, 'GET') : apiCall('/students/discipline-get', 'GET'),
        recordDiscipline: async (studentId, data) =>
            apiCall('/students/discipline-record', 'POST', { student_id: studentId, ...data }),
        ...
        resolveDiscipline: async (recordId, data) =>
            apiCall('/students/discipline-resolve', 'POST', { record_id: recordId, ...data }),
        
        // Documents
        getDocuments: async (id = null) =>
            id ? apiCall(`/students/documents-get/${id}`, 'GET') : apiCall('/students/documents-get', 'GET'),
        uploadDocument: async (formData) =>
            apiCall('/students/documents-upload', 'POST', formData, {}, { isFile: true }),
        deleteDocument: async (documentId) =>
            apiCall(`/students/documents-delete/${documentId}`, 'DELETE'),
        
        // By class/stream
        getByClass: async (id = null) =>
            id ? apiCall(`/students/by-class-get/${id}`, 'GET') : apiCall('/students/by-class-get', 'GET'),
        getByStream: async (id = null) =>
            id ? apiCall(`/students/by-stream-get/${id}`, 'GET') : apiCall('/students/by-stream-get', 'GET'),
        getRoster: async (id = null) =>
            id ? apiCall(`/students/roster-get/${id}`, 'GET') : apiCall('/students/roster-get', 'GET'),
        ...
        getImportTemplate: async () =>
            apiCall('/students/import-template', 'GET', null, {}, { isDownload: true }),
        
        // Academic Year
        getCurrentAcademicYear: async () =>
            apiCall('/students/academic-year-current', 'GET'),
        getAcademicYear: async (id = null) =>
            id ? apiCall(`/students/academic-year-get/${id}`, 'GET') : apiCall('/students/academic-year-get', 'GET'),
        ...
        setCurrentAcademicYear: async (yearId) =>
            apiCall('/students/academic-year-set-current', 'POST', { year_id: yearId }),
        updateAcademicYearStatus: async (yearId, status) =>
            apiCall('/students/academic-year-update-status', 'PUT', { year_id: yearId, status }),
        ...
        getAlumni: async (id = null) =>
            id ? apiCall(`/students/alumni-get/${id}`, 'GET') : apiCall('/students/alumni-get', 'GET'),
    ...
    academic: {
        index: async () =>
            apiCall('/academic/index', 'GET'),
        get: async (id = null) =>
            id ? apiCall(`/academic/${id}`, 'GET') : apiCall('/academic', 'GET'),
        create: async (data) =>
            apiCall('/academic', 'POST', data),
        update: async (id, data) =>
            apiCall(`/academic/${id}`, 'PUT', data),
        delete: async (id) =>
            apiCall(`/academic/${id}`, 'DELETE'),
        ...
        migrateCompetencyBaselines: async (data) =>
            apiCall('/academic/year-transition-migrate-competency-baselines', 'POST', data),
        ...
        getSchedule: async (id = null) =>
            id ? apiCall(`/academic/schedules-get/${id}`, 'GET') : apiCall('/academic/schedules-get', 'GET'),
        ...
        getCurriculumUnit: async (id = null) =>
            id ? apiCall(`/academic/curriculum-units-get/${id}`, 'GET') : apiCall('/academic/curriculum-units-get', 'GET'),
        ...
        getTopic: async (id = null) =>
            id ? apiCall(`/academic/topics-get/${id}`, 'GET') : apiCall('/academic/topics-get', 'GET'),
        ...
        getLessonPlan: async (id = null) =>
            id ? apiCall(`/academic/lesson-plans-get/${id}`, 'GET') : apiCall('/academic/lesson-plans-get', 'GET'),
        ...
        getSchemeOfWork: async (id = null) =>
            id ? apiCall(`/academic/scheme-of-work-get/${id}`, 'GET') : apiCall('/academic/scheme-of-work-get', 'GET'),
        ...
        postCustom: async (data) =>
            apiCall('/academic/custom', 'POST', data)
    },

    // Attendance endpoints
    attendance: {
        index: async () =>
            apiCall('/attendance/index', 'GET'),
        
        // Student attendance
        getStudentHistory: async (studentId, params) =>
            apiCall(`/attendance/student-history?student_id=${studentId}`, 'GET', null, params),
        getStudentSummary: async (studentId, params) =>
            apiCall(`/attendance/student-summary?student_id=${studentId}`, 'GET', null, params),
        getClassAttendance: async (classId, params) =>
            apiCall(`/attendance/class-attendance?class_id=${classId}`, 'GET', null, params),
        getStudentPercentage: async (studentId, params) =>
            apiCall(`/attendance/student-percentage?student_id=${studentId}`, 'GET', null, params),
        ...
        getStaffHistory: async (staffId, params) =>
            apiCall(`/attendance/staff-history?staff_id=${staffId}`, 'GET', null, params),
        getStaffSummary: async (staffId, params) =>
            apiCall(`/attendance/staff-summary?staff_id=${staffId}`, 'GET', null, params),
        getDepartmentAttendance: async (departmentId, params) =>
            apiCall(`/attendance/department-attendance?department_id=${departmentId}`, 'GET', null, params),
        getStaffPercentage: async (staffId, params) =>
            apiCall(`/attendance/staff-percentage?staff_id=${staffId}`, 'GET', null, params),
        ...
        create: async (data) =>
            apiCall('/attendance', 'POST', data),
        update: async (id, data) =>
            apiCall(`/attendance/${id}`, 'PUT', data),
        delete: async (id) =>
    ...
    admission: {
        index: async () =>
            apiCall('/admission/index', 'GET'),
        submitApplication: async (data) =>
            apiCall('/admission/submit-application', 'POST', data),
        uploadDocument: async (formData) =>
            apiCall('/admission/upload-document', 'POST', formData, {}, { isFile: true }),
    ...
    communications: {
        index: async () =>
            apiCall('/communications/index', 'GET'),
        ...
        getContact: async (id = null) =>
            id ? apiCall(`/communications/contact/${id}`, 'GET') : apiCall('/communications/contact', 'GET'),
        ...
        getInbound: async (id = null) =>
            id ? apiCall(`/communications/inbound/${id}`, 'GET') : apiCall('/communications/inbound', 'GET'),
        ...
        getThread: async (id = null) =>
            id ? apiCall(`/communications/thread/${id}`, 'GET') : apiCall('/communications/thread', 'GET'),
        ...
        getAnnouncement: async (id = null) =>
            id ? apiCall(`/communications/announcement/${id}`, 'GET') : apiCall('/communications/announcement', 'GET'),
        ...
        getInternalRequest: async (id = null) =>
            id ? apiCall(`/communications/internal-request/${id}`, 'GET') : apiCall('/communications/internal-request', 'GET'),
        ...
        getParentMessage: async (id = null) =>
            id ? apiCall(`/communications/parent-message/${id}`, 'GET') : apiCall('/communications/parent-message', 'GET'),
        ...
        getStaffForumTopic: async (id = null) =>
            id ? apiCall(`/communications/staff-forum-topic/${id}`, 'GET') : apiCall('/communications/staff-forum-topic', 'GET'),
        ...
        getStaffRequest: async (id = null) =>
            id ? apiCall(`/communications/staff-request/${id}`, 'GET') : apiCall('/communications/staff-request', 'GET'),
        ...
        getCommunication: async (id = null) =>
            id ? apiCall(`/communications/communication/${id}`, 'GET') : apiCall('/communications/communication', 'GET'),
        ...
        getAttachment: async (id = null) =>
            id ? apiCall(`/communications/attachment/${id}`, 'GET') : apiCall('/communications/attachment', 'GET'),
        createAttachment: async (formData) =>
            apiCall('/communications/attachment', 'POST', formData, {}, { isFile: true }),
        deleteAttachment: async (id) =>
            apiCall(`/communications/attachment/${id}`, 'DELETE'),
        
        // Group
        getGroup: async (id = null) =>
            id ? apiCall(`/communications/group/${id}`, 'GET') : apiCall('/communications/group', 'GET'),
        createGroup: async (data) =>
            apiCall('/communications/group', 'POST', data),
        updateGroup: async (id, data) =>
            apiCall(`/communications/group/${id}`, 'PUT', data),
        deleteGroup: async (id) =>
            apiCall(`/communications/group/${id}`, 'DELETE'),
        
        // Log
        getLog: async (id = null, params) =>
            id ? apiCall(`/communications/log/${id}`, 'GET') : apiCall('/communications/log', 'GET', null, params),
        createLog: async (data) =>
            apiCall('/communications/log', 'POST', data),
        
        // Recipient
        getRecipient: async (id = null) =>
            id ? apiCall(`/communications/recipient/${id}`, 'GET') : apiCall('/communications/recipient', 'GET'),
        ...
        getTemplate: async (id = null) =>
            id ? apiCall(`/communications/template/${id}`, 'GET') : apiCall('/communications/template', 'GET'),
        ...
        getWorkflowInstance: async (id = null) =>
            id ? apiCall(`/communications/workflow-instance/${id}`, 'GET') : apiCall('/communications/workflow-instance', 'GET'),
        ...
        markAsRead: async (messageId) =>
            apiCall(`/communications/communication/${messageId}`, 'PUT', { read: true }),
    ...
    finance: {
        index: async () =>
            apiCall('/finance/index', 'GET'),
        get: async (id = null) =>
            id ? apiCall(`/finance/${id}`, 'GET') : apiCall('/finance', 'GET'),
        create: async (data) =>
            apiCall('/finance', 'POST', data),
        update: async (id, data) =>
            apiCall(`/finance/${id}`, 'PUT', data),
        delete: async (id) =>
            apiCall(`/finance/${id}`, 'DELETE'),
        ...
        getPayroll: async (id = null) =>
            id ? apiCall(`/finance/payrolls-get/${id}`, 'GET') : apiCall('/finance/payrolls-get', 'GET'),
        getStaffPayments: async (payrollId, params) =>
            apiCall(`/finance/payrolls-staff-payments?payroll_id=${payrollId}`, 'GET', null, params),
        ...
        getStaffPayment: async (id = null) =>
            id ? apiCall(`/finance/payrolls-staff-payments-get/${id}`, 'GET') : apiCall('/finance/payrolls-staff-payments-get', 'GET'),
        ...
        generateReceipt: async (paymentId) =>
            apiCall('/finance/payments-generate-receipt', 'POST', { payment_id: paymentId }),
        generatePayslip: async (staffPaymentId) =>
            apiCall('/finance/payments-generate-payslip', 'POST', { staff_payment_id: staffPaymentId }),
        ...
        getStudentPaymentHistory: async (studentId, params) =>
            apiCall(`/finance/students-payment-history?student_id=${studentId}`, 'GET', null, params),
        
        // Reports
        generatePayrollReport: async (data) =>
            apiCall('/finance/reports-generate-payroll', 'POST', data),
        compareYearlyCollections: async (params) =>
            apiCall('/finance/reports-compare-yearly-collections', 'GET', null, params),
    ...
    inventory: {
        index: async () =>
            apiCall('/inventory/index', 'GET'),
        get: async (id = null) =>
            id ? apiCall(`/inventory/inventory/${id}`, 'GET') : apiCall('/inventory/inventory', 'GET'),
        create: async (data) =>
            apiCall('/inventory/inventory', 'POST', data),
        update: async (id, data) =>
            apiCall(`/inventory/inventory/${id}`, 'PUT', data),
        delete: async (id) =>
            apiCall(`/inventory/inventory/${id}`, 'DELETE'),
        ...
        getItemHistory: async (itemId, params) =>
            apiCall(`/inventory/items-history?item_id=${itemId}`, 'GET', null, params),
        
        // Categories
        listCategories: async (params) =>
            apiCall('/inventory/categories-list', 'GET', null, params),
        getCategory: async (id = null) =>
            id ? apiCall(`/inventory/categories-get/${id}`, 'GET') : apiCall('/inventory/categories-get', 'GET'),
        ...
        getLocation: async (id = null) =>
            id ? apiCall(`/inventory/locations-get/${id}`, 'GET') : apiCall('/inventory/locations-get', 'GET'),
        ...
        getSupplier: async (id = null) =>
            id ? apiCall(`/inventory/suppliers-get/${id}`, 'GET') : apiCall('/inventory/suppliers-get', 'GET'),
        ...
        getPurchaseOrder: async (id = null) =>
            id ? apiCall(`/inventory/purchase-orders-get/${id}`, 'GET') : apiCall('/inventory/purchase-orders-get', 'GET'),
        ...
        getRequisition: async (id = null) =>
            id ? apiCall(`/inventory/requisitions-get/${id}`, 'GET') : apiCall('/inventory/requisitions-get', 'GET'),
        ...
        getWorkflow: async (id = null) =>
            id ? apiCall(`/inventory/workflow-get/${id}`, 'GET') : apiCall('/inventory/workflow-get', 'GET'),
        ...
        getStock: async (params = {}) =>
            apiCall('/inventory/items-with-stock', 'GET', null, params),
        getCategories: async () =>
            apiCall('/inventory/categories-list', 'GET'),
        getSuppliers: async () =>
            apiCall('/inventory/suppliers-list', 'GET')
    },

    // Staff endpoints
    staff: {
        index: async () =>
            apiCall('/staff/index', 'GET'),
        get: async (id = null) =>
            id ? apiCall(`/staff/${id}`, 'GET') : apiCall('/staff', 'GET'),
        create: async (data) =>
            apiCall('/staff', 'POST', data),
        update: async (id, data) =>
            apiCall(`/staff/${id}`, 'PUT', data),
        delete: async (id) =>
            apiCall(`/staff/${id}`, 'DELETE'),
        
        // Profile & Details
        getProfile: async (id = null) =>
            id ? apiCall(`/staff/profile-get/${id}`, 'GET') : apiCall('/staff/profile-get', 'GET'),
        getSchedule: async (id = null) =>
            id ? apiCall(`/staff/schedule-get/${id}`, 'GET') : apiCall('/staff/schedule-get', 'GET'),
        getDepartments: async (id = null) =>
            id ? apiCall(`/staff/departments-get/${id}`, 'GET') : apiCall('/staff/departments-get', 'GET'),
        
        // Assignments
        assignClass: async (data) =>
            apiCall('/staff/assign-class', 'POST', data),
        assignSubject: async (data) =>
            apiCall('/staff/assign-subject', 'POST', data),
        getAssignments: async (id = null) =>
            id ? apiCall(`/staff/assignments-get/${id}`, 'GET') : apiCall('/staff/assignments-get', 'GET'),
        getCurrentAssignments: async (staffId) =>
            apiCall(`/staff/assignments-current?staff_id=${staffId}`, 'GET'),
        getWorkload: async (id = null) =>
            id ? apiCall(`/staff/workload-get/${id}`, 'GET') : apiCall('/staff/workload-get', 'GET'),
        initiateAssignment: async (data) =>
            apiCall('/staff/assignment-initiate', 'POST', data),
        
        // Attendance
        getAttendance: async (id = null) =>
            id ? apiCall(`/staff/attendance-get/${id}`, 'GET') : apiCall('/staff/attendance-get', 'GET'),
        markAttendance: async (data) =>
        ...
        applyLeave: async (data) =>
            apiCall('/staff/leaves-apply', 'POST', data),
        ...
        getPayslip: async (staffId, params) =>
            apiCall(`/staff/payroll-payslip?staff_id=${staffId}`, 'GET', null, params),
        getPayrollHistory: async (staffId, params) =>
            apiCall(`/staff/payroll-history?staff_id=${staffId}`, 'GET', null, params),
        ...
        downloadP9: async (staffId, year) =>
            apiCall(`/staff/payroll-download-p9?staff_id=${staffId}&year=${year}`, 'GET', null, {}, { isDownload: true }),
        downloadPayslip: async (staffId, params) =>
            apiCall(`/staff/payroll-download-payslip?staff_id=${staffId}`, 'GET', null, params, { isDownload: true }),
        exportHistory: async (staffId, params) =>
            apiCall(`/staff/payroll-export-history?staff_id=${staffId}`, 'GET', null, params, { isDownload: true }),
        
        // Performance
        getPerformanceReviewHistory: async (staffId, params) =>
            apiCall(`/staff/performance-review-history?staff_id=${staffId}`, 'GET', null, params),
        generatePerformanceReport: async (staffId, params) =>
            apiCall(`/staff/performance-generate-report?staff_id=${staffId}`, 'GET', null, params),
        getAcademicKPISummary: async (staffId, params) =>
            apiCall(`/staff/performance-academic-kpi-summary?staff_id=${staffId}`, 'GET', null, params),
        
        // Legacy support
        list: async (params = {}) =>
            apiCall('/staff', 'GET', null, params),
    ...
    transport: {
        ...
        // CRUD
        create: async (data) =>
            apiCall('/transport', 'POST', data),
        update: async (id, data) =>
            apiCall(`/transport/${id}`, 'PUT', data),
        delete: async (id) =>
            apiCall(`/transport/${id}`, 'DELETE'),
        
        // Advanced
        getRoutes: async (id = null) =>
            id ? apiCall(`/transport/routes-get/${id}`, 'GET') : apiCall('/transport/routes-get', 'GET'),
        assignRoute: async (data) =>
            apiCall('/transport/routes-assign', 'POST', data),
        getVehicles: async (id = null) =>
            id ? apiCall(`/transport/vehicles-get/${id}`, 'GET') : apiCall('/transport/vehicles-get', 'GET'),
        assignVehicle: async (data) =>
            apiCall('/transport/vehicles-assign', 'POST', data),
        getDrivers: async (id = null) =>
            id ? apiCall(`/transport/drivers-get/${id}`, 'GET') : apiCall('/transport/drivers-get', 'GET'),
    ...
    schedules: {
        index: async () =>
            apiCall('/schedules/index', 'GET'),
        get: async (id = null) =>
            id ? apiCall(`/schedules/${id}`, 'GET') : apiCall('/schedules', 'GET'),
        create: async (data) =>
            apiCall('/schedules', 'POST', data),
        update: async (id, data) =>
            apiCall(`/schedules/${id}`, 'PUT', data),
        delete: async (id) =>
            apiCall(`/schedules/${id}`, 'DELETE'),
        
        // Timetable
        getTimetable: async (id = null) =>
            id ? apiCall(`/schedules/timetable-get/${id}`, 'GET') : apiCall('/schedules/timetable-get', 'GET'),
        createTimetable: async (data) =>
            apiCall('/schedules/timetable-create', 'POST', data),
        
        // Exams
        getExam: async (id = null) =>
            id ? apiCall(`/schedules/exam-get/${id}`, 'GET') : apiCall('/schedules/exam-get', 'GET'),
        createExam: async (data) =>
            apiCall('/schedules/exam-create', 'POST', data),
        
        // Events
        getEvents: async (id = null) =>
            id ? apiCall(`/schedules/events-get/${id}`, 'GET') : apiCall('/schedules/events-get', 'GET'),
        createEvent: async (data) =>
            apiCall('/schedules/events-create', 'POST', data),
        
        // Activity schedules
        getActivity: async (id = null) =>
            id ? apiCall(`/schedules/activity-get/${id}`, 'GET') : apiCall('/schedules/activity-get', 'GET'),
        createActivity: async (data) =>
            apiCall('/schedules/activity-create', 'POST', data),
        
        // Rooms
        getRooms: async (id = null) =>
            id ? apiCall(`/schedules/rooms-get/${id}`, 'GET') : apiCall('/schedules/rooms-get', 'GET'),
        createRoom: async (data) =>
            apiCall('/schedules/rooms-create', 'POST', data),
        
        // Reports
        getReports: async (id = null) =>
            id ? apiCall(`/schedules/reports-get/${id}`, 'GET') : apiCall('/schedules/reports-get', 'GET'),
        createReport: async (data) =>
            apiCall('/schedules/reports-create', 'POST', data),
        
        // Routes
        getRoute: async (id = null) =>
            id ? apiCall(`/schedules/route-get/${id}`, 'GET') : apiCall('/schedules/route-get', 'GET'),
        createRoute: async (data) =>
        ...
        getSubjectTeachingLoad: async (subjectId) =>
            apiCall(`/schedules/subject-teaching-load?subject_id=${subjectId}`, 'GET'),
        ...
        getWorkflowStatus: async (workflowId) =>
            apiCall(`/schedules/scheduling-workflow-status?workflow_id=${workflowId}`, 'GET'),
    ...
    reports: {
        ...
        // Legacy support
        list: async (params = {}) =>
            apiCall('/reports', 'GET', null, params),
        get: async (id) =>
            apiCall(`/reports/${id}`, 'GET'),
        generate: async (data) =>
            apiCall('/reports', 'POST', data),
        ...
        getCustomReport: async (params = {}) =>
            apiCall('/reports', 'POST', params)
    },

    // Payments endpoints
    payments: {
        index: async () =>
            apiCall('/payments/index', 'GET'),
    ...
    system: {
        ...
        getLogs: async (params) =>
            apiCall('/system/logs', 'GET', null, params),
        clearLogs: async (data) =>
            apiCall('/system/logs-clear', 'POST', data),
        archiveLogs: async (data) =>
            apiCall('/system/logs-archive', 'POST', data),
        ...
        // Health
        getHealth: async () =>
            apiCall('/system/health', 'GET')
    },

    // School Config endpoints (match SchoolConfigController)
    schoolconfig: {
        index: async () =>
            apiCall('/school-config/index', 'GET'),

        get: async (id = null) =>
            id ? apiCall(`/school-config/${id}`, 'GET') : apiCall('/school-config', 'GET'),
        create: async (data) =>
            apiCall('/school-config', 'POST', data),
        update: async (id, data) =>
            apiCall(`/school-config/${id}`, 'PUT', data),
        delete: async (id) =>
            apiCall(`/school-config/${id}`, 'DELETE'),
    ...
    maintenance: {
        ...
        updateConfig: async (data) =>
            apiCall('/maintenance/config', 'POST', data)
    },
    ...
    admissions: {
        ...
        approve: async (id, data) =>
            apiCall('/students/admission-approve', 'POST', { student_id: id, ...data }),
        getStats: async () =>
            apiCall('/reports/admission-stats', 'GET')
    },
    ...
    resetPassword: {
        ...
        complete: async (token, newPassword) =>
            apiCall('/auth/reset-password', 'POST', { token, new_password: newPassword })
    }
};
